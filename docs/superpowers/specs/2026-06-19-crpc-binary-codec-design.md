...
Redesign the CRpc TCP binary protocol codec to eliminate draft-era pain points:
...
The formal protocol keeps the internal dispatch contract unchanged:
...
- Backwards compatibility with the draft wire format or other-language draft clients.
- Application-layer checksum (integrity delegated to TCP / TLS).
- Compression implementation (wire fields reserved; current release always sets `flags = 0`).
...
All binary endpoints switch to the formal wire format together:
...
No draft fallback listener. Old magic `0x5F3759DF` is retired.
...
Header includes `flags` (bit 0 = `COMPRESSED`) and `bodyOriginLen` (uncompressed protobuf size for zstd). **Current release:** `flags = 0` and `bodyOriginLen = body.Length`. Future work adds zstd + `compressThreshold` in encoder/decoder without wire changes.
...
| body | N | Raw protobuf (no compression in current release) |
...
| 20 | 4 | `bodyOriginLen` | Current: equals `body.Length`; future: uncompressed size when `COMPRESSED` |
...
### Draft → formal comparison

| Aspect | Draft (retired) | Formal |
| --- | --- | --- |
...
3. Current release: body is raw protobuf (no compress).
4. Write `magic`, `payloadLen`, header, body to `IByteBuffer`.
5. Do **not** call draft-era `encryptAndCompress`.
...
6. Current release: if `flags != 0`, treat as protocol error (future: decompress when `COMPRESSED`).
...
| Condition | Action |
| --- | --- |
| Bad magic, bad version, unknown `messageType`, `payloadLen` out of range | Close connection (same severity as draft decoder) |
...
| `encryptAndCompress(...)` | Remove from encoder path |
...
| Docs | `Doc/protocol.md` — summarize wire format and link here |
...
Algorithm: zstd (consistent with draft-era commented intent). Only body is compressed; header stays plaintext. Symmetric on client and server unless a later policy restricts direction.
...
3. **Integration** — HelloWorld RPC, server push, gateway push relay with binary frames.
...
- [ ] Implement codec types and encoder/decoder
- [ ] Remove draft-era checksum and compress/encrypt dead code
- [ ] Update server/client/gateway handlers for `CRpcMessageType`
- [ ] Update all tests and test helpers (`CreateRequest`, etc.)
- [ ] Fill `Doc/protocol.md` with wire format summary
...
None for current wire format. Compression implementation tracked as follow-up once the formal protocol ships with `flags = 0`.
