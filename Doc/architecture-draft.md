...
>
> **最后同步**：2026-06-28（A2 `RpcServiceRegistry`、`Orient.Runtime` / `Orient.Rpc` 拆分、`OrientLoop` / `OrientTask` 重命名）。相关 spec：`docs/superpowers/specs/2026-06-28-a2-rpc-service-registry-design.md`、`docs/superpowers/specs/2026-06-28-orient-runtime-rpc-split-design.md`。
...
| 调度 | MPSC mailbox + loop-owned timer + `WaitForWorkOrTimer`；见 [§9.5](#95-orientloop-调度timer-与-rpc-timeout) |
...
同进程 / 不同 loop   →  OrientLoop.InvokeAsync（**目标 API，尚未实现**）；底层通过 Post 到目标 loop，再 Post 回调用方 loop
不同进程             →  CRpcClient（或 Reference 代理）走真实 RPC
```

**选型**：能本地就不 RPC；同进程跨线程业务调用走 `InvokeAsync`（未来），底层必须 `Post`；跨进程才走 Client。
...
- 业务状态、`RpcServiceRegistry`（在 owner loop 线程访问）、pending 调用、定时器、`OrientTask` 完成回调，**只在所属 `OrientLoop` 线程上访问**。
- 跨线程的唯一合法入口是 `OrientLoop.Post(Action)`：把工作排到目标 loop 的队列里，由 loop 线程取出执行。
- DotNetty IO 线程不直接跑业务逻辑，**仅做编解码 + Post 到业务 loop**。
- 异步原语统一用 `OrientTask` / `OrientTask<T>`，**不要**在业务实现里直接用 `System.Threading.Tasks.Task`（除非显式 interop，且要 marshal 回 loop）。
- `OrientTaskCompletionSource.TrySetResult/TrySetException/TrySetCanceled` **只能在所属 loop 线程上调用**。
- **推荐**一个业务 OS 线程只驱动一个 `OrientLoop`；**Debug 构建**下 `BindToCurrentThread` 若在同一线程绑定第二个 loop 实例会抛 `InvalidOperationException`（Release 不检查）。

这些约束在 `OrientTaskCompletionSource.EnsureLoopThread`、`OrientLoop.EnsureInLoopThread` 中是强制的：
...
| 角色 | 类型 | 职责 |
| --- | --- | --- |
| 业务循环 | `OrientLoop` | 单线程 **MPSC mailbox** + **loop-owned timer**（`Orient.Runtime`，BCL only）；`Tick()` drain 动作与到期 timer；`WaitForWorkOrTimer` 阻塞等待（见 [§9.5](#95-orientloop-调度timer-与-rpc-timeout)）。**不含 RPC 类型。** |
| 循环驱动（一次性） | `OrientLoopRunner` | 给主线程 / 同步入口用：`Tick + WaitForWorkOrTimer` 直到一个 `OrientTask` 完成。 |
| 循环驱动（常驻） | `OrientLoopHost` | 服务端 / 客户端常驻：`Tick + WaitForWorkOrTimer` 直到 cancel；推荐入口为 `OrientLoopHost.RunUntilCancelled`。`CRpcLoopHost` / `CRpcClientLoopHost`（`Orient.Rpc`）为兼容别名。 |
| RPC 异步原语 | `OrientTask` / `OrientTask<T>` / `OrientTaskCompletionSource<T>` / `OrientAsyncMethodBuilder*` | 自定义 await 状态机；continuation 通过 `loop.Post` 回到 loop 线程恢复。 |
| Service 注册表 | `RpcServiceRegistry` | 由 `CRpcServer.Services` 持有；`Register` / `TryGet` / `Unregister` 均 `EnsureInLoopThread`（A2）。 |
| 服务端 | `CRpcServer` | 协议端点：持有 `Loop`、`Services`、`Connections`；`StartAsync`/`StopAsync` 返回 `OrientTask`，在 owner loop 上管理 DotNetty 监听。 |
...
| 服务端 IO 处理器 | `CRpcServerHandler` | DotNetty `ChannelHandlerAdapter`；`ChannelRead` → `server.Loop.Post` → `server.Services.TryGet`。不持有业务状态。 |
| 客户端 | `CRpcClient` | 持有 `pending calls` 表；经 `TcpChannelHost` 管理出站 TCP 连接；`CallAsync` 要求显式正数 `timeout` 并在 **owner loop timer** 上注册；连接关闭时 `FailPendingCalls`；详见 [§6.3](#63-出站传输tcpchannelhost与-crpcclient)；response/timeout/断连竞争语义见 [§9.5.8](#958-rpc-timeout-语义) / [§9.5.9](#959-pending-call-生命周期)。 |
...
1. **业务 loop 线程**
   - 由谁创建：服务端来自 `OrientLoopHost.RunUntilCancelled` 调用线程（推荐，见 `Example/HelloWorld/Server/Program.cs`）；客户端来自 `OrientLoopRunner.RunUntilComplete` 的调用线程。
   - 谁绑定：`OrientLoop.BindToCurrentThread()`，写入 `threadId` 与 `[ThreadStatic] current`。
   - 跑什么：`OrientLoop.Tick()`，即 `Services.TryGet`、`OnMessageAsync`、`CallAsync`、超时回调、`OrientTask` 的 continuation。
...
4. **DotNetty 出站客户端 IO 线程组**
   - `CRpcClient` 经 `TcpChannelHost` 创建 `MultithreadEventLoopGroup(IoThreadCount)`（默认 1），处理 connect/read/write；入站经 `LoopInboundHandler` → `PostInboundMessage` → `ownerLoop.Post`。

5. **.NET 线程池 / 调用方任意线程**
   - `OrientTask.FromTask` 用 `Task.ContinueWith` 做 interop，`ContinueWith` 的回调可能在线程池或同步线程上跑，**只允许 `loop.Post` 回业务 loop**，不能直接动业务状态。
...
```mermaid
flowchart TB
    subgraph BusinessLoopThread["业务 Loop 线程 (OrientLoop)"]
        Loop["OrientLoop<br/>actions + timers"]
    ...
    HttpSrv -.-> Loop
    Net <-- bytes --> DecS --> SH
    SH -- "Loop.Post → Services.TryGet → RpcServiceInvoker" --> Loop
...
- **Netty IO 线程 → 业务 Loop**：唯一通道是 `OrientLoop.Post`（`Server.Loop.Post` / `Client.ownerLoop.Post`）。
- **业务 Loop → Netty IO 线程**：调用 `IChannel.WriteAndFlushAsync(frame)`，**不 await 返回的 `Task`**（避免业务线程被 IO 写完成回调拖回线程池）。
- **业务 Loop 内部**：`OrientTask` await ↔ `OrientTaskCompletionSource.OnCompleted/TrySetResult` ↔ `loop.Post(continuation)`。
...
- `OrientLoop` 是 **业务执行上下文**：拥有业务线程、mailbox、timer、`OrientTask` continuation；**不含 RPC 注册表**。
- `RpcServiceRegistry` 是 **RPC service 注册表**：由 `CRpcServer.Services` 持有，所有 `Register` / `TryGet` 在 owner loop 线程执行。
- `IRpcService` 是 **业务能力和业务状态**：业务状态只在所属 `OrientLoop` 线程访问。
- `CRpcServer` 是 **协议端点**：监听端口、配置 DotNetty pipeline、管理连接生命周期与 `Services`，并把解码后的请求投递到目标 loop。
...
```text
OrientLoop 决定：业务在哪里执行，状态归谁所有（mailbox / timer / OrientTask）。
...
多协议不是多份业务状态，而是多种接入方式共享同一个业务 loop 与同一份 `CRpcServer.Services` 注册表。对外暴露时**优先同端口多种协议**（Port Unification）；双端口仅作可选部署。
...
- 共享 **一个** `OrientLoop`、**一个** `CRpcServer.Services`、**一个** `CRpcConnectionRegistry`、**一套** boss/worker IO 线程组。
...
Service 之间如何通信，取决于双方是否在同一个 `OrientLoop`、同一个进程、同一个线程。边界选型见文首 [概要 · Service 间通信](#service-间通信按边界选型)；本节给出原则、示例与代码。

总原则：

- **同线程**：直接本地接口调用。
- **同进程不同线程**：业务代码通过 `OrientLoop.InvokeAsync` 调用（**目标 API，尚未实现**）；底层通过 `OrientLoop.Post` 投递到目标 loop。
...
如果多个 Service 归属同一个 `OrientLoop`，它们的业务逻辑都在同一个 loop 线程执行。注册在 server 上（`crpcServer.Services.Register`），CRpc/HTTP 等多个端点共享同一 `CRpcServer.Services` 注册表。

这种情况下，Service 之间应当通过本地业务接口互相调用：

```csharp
var loop = new OrientLoop();
var crpcServer = new CRpcServer(loop);
...
- 返回值继续使用 `OrientTask<T>`，保持 loop 内异步模型一致。

`IRpcService.OnMessageAsync` 是外部 RPC 消息进入 Service 的传输层入口，不应作为同线程 Service 协作的主要接口。

#### 5.2 同进程不同线程：通过 `OrientLoop.InvokeAsync`（目标，尚未实现）

如果两个 Service 在同一个进程内，但分别归属不同的 `OrientLoop`，就不能直接跨线程访问对方的业务状态。

底层边界仍然是 `OrientLoop.Post`：请求先投递到目标 loop，目标 loop 执行业务逻辑，完成后再投递回调用方 loop 完成结果。业务代码不应手写 `Post + OrientTaskCompletionSource + 再 Post 回调用方` 的逻辑，未来应通过 `OrientLoop.InvokeAsync` 调用：

```csharp
// 异步 + 有返回值
var user = await OrientLoop.InvokeAsync(
    userLoop,
    () => userService.GetUserAsync(userId));
```

`InvokeAsync` 是**目标**框架级跨 loop 调度原语（Runtime 尚未实现），其接口为：
...
// 异步 + 无返回值
await OrientLoop.InvokeAsync(
    userLoop,
    () => userService.RefreshAsync());

// 同步 + 有返回值
var count = await OrientLoop.InvokeAsync(
    userLoop,
...
- 必须从调用方 `OrientLoop` 线程调用；返回的 `OrientTask` 归属调用方 loop。
- `targetLoop` 决定 `action` 在哪个 loop 上执行。
- `callerLoop == targetLoop` 时可直接执行 `action`，不必再绕 mailbox。
- `action` 抛出的异常，或其返回的 `OrientTask` faulted / canceled，应传播到调用方 loop 上完成的返回任务。

更友好的 `UserServiceLocalRef` 可以建立在 `InvokeAsync` 上。它只负责绑定目标 loop 和目标 service，不重复实现跨 loop 调度逻辑：
...
public sealed class UserServiceLocalRef
{
    ...
    public UserServiceLocalRef(OrientLoop targetLoop, UserServiceImpl service)
    {
...
- `OrientLoop.Post` 仍是跨线程入口，内部队列必须线程安全；`InvokeAsync` 只是把双向 `Post` 封装起来。
- 目标 Service 的业务状态只在目标 loop 线程访问。
- 调用方的 `OrientTaskCompletionSource` 只在调用方 loop 线程完成。
...
- 同进程 / 不同 loop：`OrientLoop.InvokeAsync`（未来）封装跨 loop 消息投递，底层仍通过 `OrientLoop.Post`；protobuf 参数/返回值默认 `Clone()` 成快照，通常不需要 `ToByteArray()` 序列化。
- 不同进程：`CRpcClient` RPC 调用，需要序列化，也需要网络错误与超时处理。

一句话总结：

**能本地调用就不要 RPC；同进程跨线程业务调用走 `InvokeAsync`（未来），底层必须 `Post`；跨进程才走 `CRpcClient`。**
...
```6:42:Orient.Runtime/Loop/OrientLoop.cs
...
- `timerScheduler` 是 **loop-owned timer**（默认 `MinHeapTimerScheduler`）；`ScheduleDelay` / `ScheduleAt` 为 internal，由 `Orient.Rpc` 等在 owner loop 线程调用。
...
- `Loop` 在构造时**必须显式传入**；`Services` 与 `Connections` 在构造时创建并绑定同一 loop。
- **推荐启动形态**（HelloWorld）：在 `OrientLoopRunner.RunUntilComplete` 中 `crpcServer.Services.Register(impl)` → `await crpcServer.StartAsync`（+ 可选 HTTP），再 `OrientLoopHost.RunUntilCancelled` 常驻驱动，退出时回到 loop 内 `await StopAsync`。
- `StartAsync` / `StopAsync` 返回 `OrientTask`，必须在 owner loop 线程调用；DotNetty `BindAsync` / `CloseAsync` / `ShutdownGracefullyAsync` 经 `OrientTask.FromTask(..., Loop)` 回到 loop 后再修改端点状态。
- `RunAsync` 也返回 `OrientTask`，内部完成启动后嵌入 `OrientLoopHost.RunUntilCancelled`；**不再**在退出时清 registry（见 `docs/superpowers/specs/2026-06-27-crpc-server-lifecycle-design.md`）。
...
`CRpcServerHandler` 是 IO→Loop 搬运工：IO 线程 `Post` 到 loop，在 loop 线程 `Services.TryGet`，再经 `RpcServiceInvoker` 调业务：
...
`HttpServerHandler` / 应用层 HTTP 可走同一路径：解析 HTTP/JSON 后在 loop 线程 `Services.TryGet` + `RpcServiceInvoker.InvokeAsync`；HelloWorld 的 `GreeterHttpHandler` 亦可 `loop.Post` 后直接调 typed 方法。
...
| 层 | 类型 | 职责 |
| --- | --- | --- |
| 传输宿主 | `TcpChannelHost` | 单连接 lifecycle；`LoopInboundHandler` 将入站 Post 到 owner loop；可选 `sharedEventLoopGroup`（`ownsEventLoopGroup == false` 时 dispose 不 shutdown group，见 [§8.1](#81-io-线程组与业务-loop-的拓扑)） |
...
- 连接状态由 `TcpChannelHost` 持有；`ConnectAsync` / `CloseAsync` 在 owner loop 线程调用；DotNetty connect/close 经 `OrientTask.FromTask` 回到 loop。
- 入站：`LoopInboundHandler` → `host.PostInboundMessage` → `ownerLoop.Post` → `InboundMessageReceived` → `OnHostInboundMessage` → `CompleteReceiveResponse`。
- `ownerLoop` 在构造时显式绑定；`ConnectAsync` / `CloseAsync` / `CallAsync` 均要求 `OrientLoop.Current` 与 owner 为同一实例。
...
#### 6.4 OrientLoopHost / OrientLoopRunner

- **现状**：`Tick + WaitForWorkOrTimer`。
- `OrientLoopHost.RunUntilCancelled(loop, ct)`：服务端 / 客户端推荐常驻驱动入口（`Orient.Runtime`）。
- `CRpcLoopHost` / `CRpcClientLoopHost`（`Orient.Rpc`）：thin alias，转发到 `OrientLoopHost`。
- `OrientLoopRunner.RunUntilComplete(loop, op)`：在调用线程 binding loop，跑到指定 `OrientTask` 完成；适合 main 线程一次性脚本（如 HelloWorld Client / Server 启动段）。
...
sequenceDiagram
    ...
    participant BizLoop as OrientLoop<br/>业务线程
    ...
    BizLoop->>BizLoop: Services.TryGet + RpcServiceInvoker
...
sequenceDiagram
    ...
    participant BizLoop as OrientLoop
    participant Net as Socket
    participant Dec as CRpcMessageDecoder<br/>Netty IO 线程
...
sequenceDiagram
    ...
    participant Host as TcpChannelHost
    participant Cli as CRpcClient
    participant BizLoop as OrientLoop<br/>owner
    ...
    BizLoop-->>App: pending OrientTask TrySetException
```
...
- 业务 loop 线程来自 `OrientLoopHost.RunUntilCancelled` 调用线程（推荐）或 `RunAsync` 内嵌驱动（遗留）。
- 后果：
  - 没有"按业务模块切多个 `OrientLoop`"的路由能力，整个进程默认可视为单 loop 单线程业务吞吐。
...
- 生命周期 API（`ConnectAsync` / `CloseAsync` / `ShutdownIoAsync`）已统一为 `OrientTask`；DotNetty `.NET Task` 仅作 IO interop，completion 必须回到 owner loop。
...
- `CRpcServer.StartAsync` / `StopAsync` 与 `HttpServer.StartAsync` / `StopAsync` 已统一为 `OrientTask`，必须在 owner loop 线程调用。
- `bootstrapChannel`、IO groups、运行状态的写入收束到 owner loop；对外 `IsRunning` 是 `Volatile` 快照。
- `CRpcServer.RunAsync` 仅作为 demo host helper 保留；生产组合为 `StartAsync` + `OrientLoopHost.RunUntilCancelled` + `StopAsync`（`docs/superpowers/specs/2026-06-27-crpc-server-lifecycle-design.md`）。

#### 8.5 `OrientLoopRunner.RunUntilComplete` 的使用方式
...
- 已提供 `CRpcReference` 与 `CRpcClientLoopHost`（alias）；`RunUntilComplete` 适合 main 线程一次性脚本，常驻客户端用 `OrientLoopHost` 或 `CRpcClientLoopHost.RunUntilCancelled`。

#### 8.6 业务回调里 `Console.WriteLine` 当作可观测性

- `CRpcServerHandler` / `CRpcClient` 等处有大量 `Console.WriteLine($"*******…");`（`LoopInboundHandler` 本身无日志）。
...
flowchart LR
    ...
    subgraph Business["业务层 (N 个 OrientLoop)"]
        L1[OrientLoop A<br/>User分片 1]
        L2[OrientLoop B<br/>User分片 2]
        L3[OrientLoop C<br/>聊天]
    end
...
- 每个 `CRpcServer` 拥有自己的 `RpcServiceRegistry`（绑定一个 loop）；多 server 实例可共享 loop 或按路由投递到不同 loop。
...
```csharp
// OrientLoop: 可唤醒 mailbox + loop-owned timer（无 RPC 类型）
public sealed class OrientLoop {
...
public static class OrientLoopHost {
    public static void RunUntilCancelled(OrientLoop loop, CancellationToken ct);
}

public static class OrientLoopRunner {
    public static T RunUntilComplete<T>(OrientLoop loop, Func<OrientTask<T>> op);
}

// Server / Client 显式注入 loop
public sealed class CRpcServer {
    public CRpcServer(OrientLoop loop, CRpcServerOptions opts);
    public OrientLoop Loop { get; }
    public RpcServiceRegistry Services { get; }
...
public sealed class HttpServer {
    public HttpServer(OrientLoop loop, HttpServerOptions opts);
    public OrientTask StartAsync(CancellationToken ct = default);
    public OrientTask StopAsync();
}
public sealed class CRpcClient {
    public CRpcClient(OrientLoop loop, CRpcClientOptions opts); // 显式 loop
}
```
...
// 或 CRpcClientLoopHost.RunUntilCancelled(loop, cancellationToken);  // alias
```

`CRpcReference` 是业务入口；`CRpcClient` 是底层 transport client，仍负责连接、pending call、request sequence、超时和响应分发。Service 内部调用其它进程时也使用生成代理，但不能创建或驱动新的 loop，必须复用当前 `OrientLoop.Current`。

#### 9.4 关键不变量（重申）

1. 业务状态 / pending 调用表 / `RpcServiceRegistry` / **client `channel` 连接状态** / endpoint 生命周期状态，**只在所属 loop 线程访问**。
...
7. **Endpoint 生命周期 CRpc 化**：`CRpcServer` / `HttpServer` 的 `StartAsync` / `StopAsync`，以及 `CRpcClient` 的 `ConnectAsync` / `CloseAsync` / `ShutdownIoAsync` 返回 `OrientTask`；DotNetty `.NET Task` 只做 IO interop，状态变更 completion 回到 owner loop。

#### 9.5 OrientLoop 调度、Timer 与 RPC Timeout

> 本节覆盖 OrientLoop 调度、通用 timer 与 `CRpcClient` RPC timeout 的一致性语义。
...
| Loop-owned | Timer 无论底层用 min-heap 还是 timing wheel，**永远归属当前 `OrientLoop`**，只在 `Tick()` 里推进，不另开全局 timer 线程 |
| Absolute deadline | 内部统一按 **absolute due timestamp**（`Stopwatch.GetTimestamp()`）排序；`ScheduleDelay(ms)` 只是便捷封装 |
| Driver 薄 | `OrientLoopHost` / `CRpcLoopHost` / `CRpcClientLoopHost` / `OrientLoopRunner` 使用 `Tick + WaitForWorkOrTimer`；空闲时阻塞，有 `Post` 或 timer 到期时唤醒 |

##### 9.5.2 Mailbox：MPSC 语义

`OrientLoop.Post` / `Tick` 之间的 mailbox **语义是 MPSC**，不是 MPMC：
...
```csharp
// 边界稳定时可替换 backend，OrientLoop 对外 API 不变
internal interface IOrientLoopMailbox
{
...
**Driver 兜底**：`OrientLoopHost` / `OrientLoopRunner` 在 `loop.Tick()` 外另有 try/catch，防止仍有异常逃出时终止 driver 线程（打 stderr 后继续循环）。
...
**测试**：`Tests/Orient.Tests/OrientLoopExceptionIsolationTests.cs`（action / timer 连续执行、事件上报、无 handler、handler 自身抛异常）。
...
因此 **不在 `IOrientLoopTimerScheduler` 上暴露 `GetNextDueTimestamp()`**，避免两个 API 表达同一件事、且 timing wheel 难以统一实现。若 min-heap 实现需要堆顶 deadline，作为 `MinHeapTimerScheduler` 内部字段即可。
...
`OrientLoopOptions` 可预留 `TimerSchedulerFactory`，默认 `() => new MinHeapTimerScheduler()`。Public API 不暴露 backend 细节，业务只调用 `ScheduleDelay` / `ScheduleAt`。
...
| --- | --- |
| 同线程 / 同 `OrientLoop` 本地接口调用 | **否** — 普通本地调用，无 pending call 表、无 seq、无 IO `Post` |
...
- **主动**：`CloseAsync` 在 owner loop 线程调用 `FailPendingCalls`，再经 `TcpChannelHost.CloseAsync` 关闭 DotNetty channel。
- **被动**：`LoopInboundHandler.ChannelInactive`（IO 线程）→ `TcpChannelHost.PostChannelInactive` → `ownerLoop.Post(OnHostChannelInactive)` → `FailPendingCalls`。
...
`OrientLoopHost` / `OrientLoopRunner` 同样在 `Tick()` 外 try/catch。业务异常正常在 `Tick` 内隔离（见 [§9.5.5](#955-tick-异常隔离)）；外层 catch 仅作兜底。

`OrientLoopRunner.RunUntilComplete` 同理：`Tick + WaitForWorkOrTimer` 直到目标 `OrientTask` 完成。
...
1. **`OrientLoop.InvokeAsync`**：跨 loop 调度原语（见 §5.2）。
2. **`RouteLoop` 钩子**：`CRpcServer` / `HttpServer` 增加 `Func<..., OrientLoop>? RouteLoop`，支持按消息路由到不同 loop（见 §9.2）。
3. **共享 IO group 注入**：`TcpChannelHost` 已支持注入共享 group；`CRpcServerOptions` / `CRpcClientOptions` 对 CRPC 服务端与 Reference 客户端暴露注入仍待做。
4. **多协议示例变体**：HelloWorld 默认即为纯 CRpc；已支持 `--unified`（**推荐**，单端口 Port Unification）与 `--http`（可选，CRpc 7999 + HTTP 8080）；仍缺管理端口、第二个 CRpc 端口等变体示例。
