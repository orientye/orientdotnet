...
| 客户端 | `CRpcClient` | 持有 `pending calls` 表 + DotNetty `Bootstrap`；`CallAsync` 要求显式正数 `timeout` 并在 **owner loop timer** 上注册；连接关闭时 `FailPendingCalls`；response/timeout/断连竞争语义见 [§9.5.7](#957-rpc-timeout-语义) / [§9.5.8](#958-pending-call-生命周期)。 |
...
> Response 经 IO 线程 `Post` 回 owner loop；与 timeout / 断连的竞争规则见 [§9.5.7 RPC Timeout 语义](#957-rpc-timeout-语义) 与 [§9.5.8 Pending Call 生命周期](#958-pending-call-生命周期)。
