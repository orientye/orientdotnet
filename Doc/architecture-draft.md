...
同进程 / 不同 loop   →  CRpcLoop.InvokeAsync；底层通过 Post 到目标 loop，再 Post 回调用方 loop
...
- **同线程**：直接本地接口调用。
- **同进程不同线程**：业务代码通过 `CRpcLoop.InvokeAsync` 调用；底层通过 `CRpcLoop.Post` 投递到目标 loop。
...
#### 5.2 同进程不同线程：通过 `CRpcLoop.InvokeAsync`
...
```csharp
// 异步 + 有返回值
var user = await CRpcLoop.InvokeAsync(
    userLoop,
...
public static CRpcTask<T> InvokeAsync<T>(
    CRpcLoop targetLoop,
    Func<CRpcTask<T>> action);

public static CRpcTask InvokeAsync(
    CRpcLoop targetLoop,
    Func<CRpcTask> action);
...
// 异步 + 无返回值
await CRpcLoop.InvokeAsync(
    userLoop,
    () => userService.RefreshAsync());

// 同步 + 有返回值
var count = await CRpcLoop.InvokeAsync(
    userLoop,
...
- 必须从调用方 `CRpcLoop` 线程调用；返回的 `CRpcTask` 归属调用方 loop。
- `targetLoop` 决定 `action` 在哪个 loop 上执行。
- `callerLoop == targetLoop` 时可直接执行 `action`，不必再绕 mailbox。
- `action` 抛出的异常，或其返回的 `CRpcTask` faulted / canceled，应传播到调用方 loop 上完成的返回任务。
...
{
    private readonly CRpcLoop targetLoop;
    private readonly UserServiceImpl service;

    public UserServiceLocalRef(CRpcLoop targetLoop, UserServiceImpl service)
    {
        this.targetLoop = targetLoop;
        this.service = service;
    }

    public CRpcTask<UserInfo> GetUserAsync(long userId)
    {
...
`TcpChannelHost`（`CRpc/Transport/TcpChannelHost.cs`）是**出站、单 TCP 连接**的 DotNetty 传输层：`Bootstrap`、`ConnectAsync` / write / close、可选 IO group 生命周期。服务端 listen/accept **不走**这一层——`CRpcServer` / `HttpServer` 各自持 `ServerBootstrap`（见 [§6.2](#62-crpcserver--crpcserverhandler--rpcserviceinvoker)）。
