...
> 本文聚焦：**线程、CRpcLoop、CRpcServer / CRpcServerHandler、CRpcClient / CRpcClientHandler，以及它们之间的关系**。
> 既描述现状，也指出现有实现的问题，并给出目标方向。**现有代码 ≠ 推荐设计**。
...
{
    CRpcTask<UserInfo> GetUserAsync(long userId);
}

public sealed class OrderService : OrderBase
{
    private readonly IUserServiceLocal userService;

    public OrderService(IUserServiceLocal userService)
    {
