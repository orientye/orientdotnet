...
同进程 / 不同 loop   →  OrientLoop.InvokeAsync（`Orient.Runtime/Loop/OrientLoop.InvokeAsync.cs`）；底层通过 Post 到 target loop，再 Post 回 caller loop
...
- **同线程**：直接本地接口调用。
- **同进程不同线程**：业务代码通过 `OrientLoop.InvokeAsync` 调用；底层通过 `OrientLoop.Post` 投递到 target loop。
...
#### 5.2 同进程不同线程：通过 `OrientLoop.InvokeAsync`

如果两个 Service 在同一个进程内，但分别归属不同的 `OrientLoop`，就不能直接跨线程访问对方的业务状态。

底层边界仍然是 `OrientLoop.Post`：请求先投递到 target loop，target loop 执行业务逻辑，完成后再投递回 caller loop 完成结果。业务代码不应手写 `Post + OrientTaskCompletionSource + 再 Post 回 caller` 的逻辑，应通过 `OrientLoop.InvokeAsync` 调用：
...
`InvokeAsync` 是 Runtime 提供的跨 loop 调度原语（`Orient.Runtime/Loop/OrientLoop.InvokeAsync.cs`），其接口为：
...
- 同进程 / 不同 loop：`OrientLoop.InvokeAsync` 封装跨 loop 消息投递，底层仍通过 `OrientLoop.Post`；protobuf 参数/返回值默认 `Clone()` 成快照（由 LocalRef / codegen 负责，尚未实现），通常不需要 `ToByteArray()` 序列化。
